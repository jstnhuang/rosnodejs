...
var ActionClientInterface = function (_EventEmitter) {
  ...
  function ActionClientInterface(options) {
    ...
    var goalOptions = Object.assign({ queueSize: 10, latching: false }, options.goal);
    _this._goalPub = nh.advertise(_this._actionServer + '/goal', _this._actionType + 'ActionGoal', goalOptions);

    var cancelOptions = Object.assign({ queueSize: 10, latching: false }, options.cancel);
    ...
    _this._feedbackSub = nh.subscribe(_this._actionServer + '/feedback', _this._actionType + 'ActionFeedback', function (msg) {
      _this._handleFeedback(msg);
    }, feedbackOptions);

    var resultOptions = Object.assign({ queueSize: 1 }, options.result);
    _this._resultSub = nh.subscribe(_this._actionServer + '/result', _this._actionType + 'ActionResult', function (msg) {
  ...
  _createClass(ActionClientInterface, [{
    ...
    value: function cancel(goalId) {
      var cancelGoal = new GoalID({ stamp: Time.now() });
      if (!goalId) {
